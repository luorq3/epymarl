...
do
  ...
  python src/main.py --config=${config} --env-config=${env_config} with env_args.time_limit=${time_limit} env_args.key="${key}" env_args.scenario="${key}" seed="${seed}" runner=${runner} batch_size_run=${batch_size_run} remark=${job_type} t_max=${t_max} use_rnn=${use_rnn}
...
do
  echo "Started with key=${key} and seed=$seed."
  python src/main.py --config=${config} --env-config=${env_config} with env_args.time_limit=${time_limit} env_args.key="${key}" env_args.scenario="${key}" seed="${seed}" runner=${runner} batch_size_run=${batch_size_run} remark=${job_type} enable_td_lambda=${enable_td_lambda} t_max=${t_max} use_rnn=${use_rnn}
  echo "Done with key=${key} and seed=$seed."
done
